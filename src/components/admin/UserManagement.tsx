...
import { FaUserPlus, FaToggleOn, FaToggleOff, FaEnvelope, FaLock, FaUser, FaPhone, FaCalendarAlt, FaMapMarkerAlt, FaTimes } from 'react-icons/fa';
import Button from '../Button';
import { getAllUsers, createUser, toggleUserActiveStatus, UserResponse, CreateUserRequest } from '../../services/userService';
...
const UserManagement: React.FC<UserManagementProps> = ({ onAlert }) => {
  ...
  const handleProvinceChange = async (provinceCode: string) => {
    setFormData(prev => ({ ...prev, provinceCode: parseInt(provinceCode) || 0, districtCode: 0 }));
    try {
  ...
  const handleToggleStatus = async (userId: string, currentStatus: boolean) => {
    try {
      ...
      onAlert('success', `User ${!currentStatus ? 'activated' : 'deactivated'} successfully`);
    } catch (error: any) {
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id}>
                  <td>{user.email}</td>
                  <td>
                    <span className={`badge status-${user.emailVerified ? 'verified' : 'unverified'}`}>
        ...
        <div className="modal-overlay">
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h3 id="modal-title"><FaUserPlus /> Create New User</h3>
              <button 
                type="button" 
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleCreateUser(); }} className="beautiful-form">
              ...
              <div className="form-group">
                <label htmlFor="birthDate"><FaCalendarAlt /> Birth Date</label>
                <input
                  id="birthDate"
                  type="date"
              ...
              <div className="form-group">
                <label htmlFor="provinceCode"><FaMapMarkerAlt /> Province</label>
                <select
                  id="provinceCode"
              ...
              <div className="form-group">
                <label htmlFor="districtCode"><FaMapMarkerAlt /> District</label>
                <select
                  id="districtCode"
                  value={formData.districtCode || ''}
                  onChange={(e) => setFormData(prev => ({ ...prev, districtCode: parseInt(e.target.value) || 0 }))}
                  required
                  className="user-form-select"
